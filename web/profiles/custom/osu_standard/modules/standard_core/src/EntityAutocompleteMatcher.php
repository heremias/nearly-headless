      ...
      foreach ($entity_labels as $values) {
        foreach ($values as $entity_id => $label) {
          ...
          if ($entity->getEntityType()->id() == 'node') {
            $updated = \Drupal::service('date.formatter')->format($entity->getChangedTime(), 'custom', 'm/d/y H:i A');
          }
          ...
          $key = Tags::encode($key);
          $label = $label . ' (' . $entity_id . ') <div style="float: right; padding-right: 1em;">' . $updated . '</div>';
