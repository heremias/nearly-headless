...
class PersonProcessor extends ProcessorBase {
  ...
  public static function extract($node, $mode) {
    ...
    if (in_array($mode, ['full', 'teaser']) && $node->hasField('site_tags')) {

      // Load our vocabularies.
      $vocabs = self::modeVocabularies($mode);
      ...
      if ($vocabs) {
        foreach ($vocabs as $vocab) {
          ...
          foreach ($node->site_tags as $t) {
            if ($t->entity && ($t->entity->bundle() == $vocab->id())) {
              $term = [
                'id' => $t->entity->id(),
                'name' => $t->entity->label()
              ];
   ...
   */
  public static function modeVocabularies($mode) {
    $config = \Drupal::config('standard_core.developer_settings');
    $vocab_ids = explode(',', $config->get("person_{$mode}_vocabs"));
